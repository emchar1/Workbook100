...
class CollectionCellGloves: UICollectionViewCell {
    // MARK: - Properties
    ...
    static let reuseID: String = "CollectionCellGloves"
    ...
    func setViews(with model: CollectionModel) {
        ...
        labelTitle.text = model.productNameDescription + " " + model.colorway
...
//        var rightReturn = ""
        ...
        for (i, size) in model.sizes.enumerated() {
            ...
            leftReturn += "\(model.sizes[i].size ?? "S"):\t\(model.sizes[i].colorwaySKU ?? "00000-00000")\n"
        }
